...
pub struct Props<T: Clone + PartialEq>
{
    ...
    pub data_to_html_panel: Callback<(usize, T), Html>,
    pub data_ref: UseStateHandle<Vec<T>>,
...
pub fn list<T: Clone + PartialEq>(props: &Props<T>) -> Html
{
    html!
    {
        <div class={props.class.clone()} style={props.style.clone()}>
            ...
            {
                props.data_ref.iter().enumerate().map(
                    |(i, t)| { props.data_to_html_panel.emit((i, t.clone())) }
                ).collect::<Html>()
            }
            </div>
        </div>
